...
public class ViewMemory extends JPanel {
    ...
    private int offset; // To show real addresses (e.g. $8000 for ROM)
    ...
    public ViewMemory(int size, int addressOffset) {
        ...
        // Define Columns
        String[] columnNames = {"Address", "Hex Value", "Binary"};
        model = new DefaultTableModel(columnNames, 0) {
            @Override
            public boolean isCellEditable(int row, int column) {
                return false; // No Typing on Cells
            }
        };
        ...
        // we only load 256 rows for now
        // needs to be updated later for dynamic loading
        ...
        scrollPane.getViewport().setBackground(new Color(30, 30, 30)); // match table bg
        scrollPane.setBorder(BorderFactory.createEmptyBorder()); // no border
        add(scrollPane, BorderLayout.CENTER); // add to panel
    }
